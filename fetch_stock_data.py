...
def main(symbol_file, parameter_file, output_dir):
	...
	'''
	param_fp = open('input\params.txt', 'r') ##used to be parameter_file
	...
	with open('input\symbols.txt', 'r') as sym_fp: ##used to be symbols_file
		for symbol in list(csv.reader(sym_fp)):
			...
			new_file = open(os.path.join('scripts', str(symbol) + '.csv'), 'w+') ## used to be output_dir
...
##  File "c:\Users\kolby\Documents\Python\StockPricePrediction-master\scripts\fetch_stock_data.py", line 116, in <module>
##   main(str(sys.argv[1]), str(sys.argv[2]), str(sys.argv[3]))
